  ...
  return (
      ...
      <div className="profileTab">
        <IdContext.Provider value={id}>
          ...
          <ContentBox heading={"Phones and Emails"} addButton={true} boxId={"contact"} handleChange={handleChange}>
            {currentClient &&
              currentClient.phone && 
                currentClient.phone.map((item, index) => {
                  return (
                    <ContentRow edit={true} del={true} copy={true}  value={item} handleChange={handleChange} rowId={'phone'} key={index}>
                      <ContentItem value={item} label={"Mobile"}></ContentItem>
                    </ContentRow>
                  );
                  })}

                <ContentRow edit={true} del={false} copy={true} value={currentClient.username} handleChange={handleChange} rowId={'username'}>
            ...
            {currentClient &&
              currentClient.email && 
                currentClient.email.map((item, index) => {
                  return (
                    <ContentRow  edit={true} del={true} copy={true}  value={item} handleChange={handleChange} rowId={'email'} key={index}>
          ...
          <ContentBox heading={"Emergency Contact"} addButton={true} buttonLabel={"Contact"} boxId={'emergency'}>
            <ContentRow edit={true} del={true}>
              <ContentItem value={"252-333-4102"} label={"Mobile"}/>
              <ContentItem value={"Type of contact"} label={"Relationship"}/>
            </ContentRow>
          </ContentBox>
        </IdContext.Provider>
      </div>
